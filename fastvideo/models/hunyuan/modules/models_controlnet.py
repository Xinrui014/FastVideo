...
# override
class HYVideoDiffusionTransformerControlNet(HYVideoDiffusionTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # factory_kwargs = {"device": device, "dtype": dtype}

        # Copy the first 20 double blocks and freeze them
        # self.fixed_double_blocks = nn.ModuleList([block.to(self.device) for block in self.double_blocks[:20]])
        # for param in self.fixed_double_blocks.parameters():
        #     param.requires_grad = False
        # self.fixed_double_blocks = nn.ModuleList([
        #     MMDoubleStreamBlock(
        #         self.hidden_size,
        #         self.heads_num,
        #         mlp_width_ratio=self.config.mlp_width_ratio,
        #         mlp_act_type=self.config.mlp_act_type,
        ...
        # Create a parallel set of double blocks
        self.parallel_double_blocks = nn.ModuleList([
            MMDoubleStreamBlock(
                self.hidden_size,
                self.heads_num,
                mlp_width_ratio=self.config.mlp_width_ratio,
    ...
    def forward(
        ...
        if len(self.single_blocks) > 0:
            for j, block in enumerate(self.single_blocks):
                single_block_args = [
                    x,
                    vec,
                    txt_seq_len,
