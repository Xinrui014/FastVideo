...
def prepare_sequence_parallel_data_LR(hidden_states, lr_hidden_states, encoder_hidden_states,
                                   attention_mask, encoder_attention_mask):
    if nccl_info.sp_size == 1:
        return (
            hidden_states,
    ...
    def prepare_LR(hidden_states, lr_hidden_states, encoder_hidden_states, attention_mask,
                encoder_attention_mask):
        hidden_states = all_to_all(hidden_states, scatter_dim=2, gather_dim=0)
        lr_hidden_states = all_to_all(lr_hidden_states, scatter_dim=2, gather_dim=0)
        encoder_hidden_states = all_to_all(encoder_hidden_states,
                                           scatter_dim=1,
                                           gather_dim=0)
        attention_mask = all_to_all(attention_mask,
    ...
    return hidden_states, lr_hidden_states, encoder_hidden_states, attention_mask, encoder_attention_mask
...
def sp_parallel_dataloader_wrapper_LR(dataloader, device, train_batch_size,
                                   sp_size, train_sp_batch_size):
    while True:
        for data_item in dataloader:
            latents, lr_latents, cond, attn_mask, cond_mask = data_item
            latents = latents.to(device)
            lr_latents = lr_latents.to(device)
            cond = cond.to(device)
            attn_mask = attn_mask.to(device)
            cond_mask = cond_mask.to(device)
            frame = latents.shape[2]
            if frame == 1:
                yield latents, lr_latents, cond, attn_mask, cond_mask
            else:
                latents, lr_latents, cond, attn_mask, cond_mask = prepare_sequence_parallel_data_LR(
                    latents, lr_latents, cond, attn_mask, cond_mask)
                assert (
                    train_batch_size * sp_size >= train_sp_batch_size
                ), "train_batch_size * sp_size should be greater than train_sp_batch_size"
                for iter in range(train_batch_size * sp_size //
